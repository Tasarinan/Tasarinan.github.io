...
next:
  text: '第9章 远程访问与网络'
  link: '/hello-claw/adopt/09-远程访问与网络'
---
...
> **前置条件**：已完成[第二章 OpenClaw 快速安装](/hello-claw/adopt/02-OpenClaw快速安装)，Gateway 已安装并运行。
...
配置文件为 JSON5 格式（支持注释和尾逗号），结构详见[附录 G](/hello-claw/appendix/G-配置文件详解)。
...
| 直接编辑（不推荐） | 手动改 JSON 容易出错，详见[附录 G](/hello-claw/appendix/G-配置文件详解) |
...
更多配置模板见[附录 G 配置文件详解](/hello-claw/appendix/G-配置文件详解)。
...
A: 详见[第五章 模型管理](/hello-claw/adopt/05-模型管理)。简短版：`models.mode: "merge"` + `model.fallbacks`，本地挂掉自动切云端。
