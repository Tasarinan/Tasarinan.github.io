...
next:
  text: '第4章 聊天平台接入'
  link: '/hello-claw/adopt/04-聊天平台接入'
---
...
> **前置条件**：已完成[第二章 OpenClaw 手动安装](/hello-claw/adopt/02-OpenClaw快速安装)。

## 0. 两条路径，选一条

**Onboarding（配置向导）** 帮你告诉龙虾三件事：用哪个模型、通过哪个渠道联系你、在哪里工作。OpenClaw 提供两条路径：

![OpenClaw Onboarding](/hello-claw/adopt/images/chapter3/OpenClaw-Onboarding.png)
...
选择模型提供商，粘贴 API Key。不确定选哪个？见[第五章 模型管理](/hello-claw/adopt/05-模型管理)。
...
- 详见[第十章 安全防护](/hello-claw/adopt/10-安全防护与威胁模型)
...
选择要连接的聊天平台（WhatsApp、Telegram、Discord 等）。可以跳过，后续用 `openclaw channels add` 随时添加，详见[第四章](/hello-claw/adopt/04-聊天平台接入)。
...
启动 Gateway、验证运行状态，安装推荐技能。详见[附录 D：技能开发与发布指南](/hello-claw/appendix/D-技能开发与发布)。
...
- **默认定位**：个人助手，一个可信操作者边界
- **多用户场景**：需要拆分信任边界、最小化工具权限，参考[第十章 安全防护](/hello-claw/adopt/10-安全防护与威胁模型)
...
安装全局 `openclaw` CLI，让终端命令和定时任务正常工作。已通过[第二章](/hello-claw/adopt/02-OpenClaw快速安装)安装过的可跳过。
...
详见[附录 G：配置文件详解](/hello-claw/appendix/G-配置文件详解)。
...
配置文件损坏时，运行 `openclaw doctor` 自动诊断并修复。详见[第八章](/hello-claw/adopt/08-网关运维)。
...
配置文件存储在 `~/.openclaw/` 目录下，工作区在 `~/.openclaw/workspace/`。配置文件结构详见[附录 G](/hello-claw/appendix/G-配置文件详解)。
...
详见[第十一章](/hello-claw/adopt/11-Web界面与客户端)。
...
不会。它只配置本地客户端如何连接远程 Gateway（SSH 隧道或 Tailnet），远程主机不受影响。详见[第九章](/hello-claw/adopt/09-远程访问与网络)。
