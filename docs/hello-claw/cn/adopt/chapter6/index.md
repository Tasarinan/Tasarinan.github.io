...
prev:
  text: '第5章 模型管理'
  link: '/hello-claw/cn/adopt/chapter5'
next:
  text: '第7章 工具与定时任务'
  link: '/hello-claw/cn/adopt/chapter7'
---
...
> **AutoClaw 用户**：[AutoClaw](/hello-claw/cn/adopt/chapter1/) 已预配置好，开箱即用。本章帮助你按需进阶调整。
...
> 通过 CLI 修改配置后，大部分设置自动热更新生效。只有端口、认证、TLS 等基础设施字段需要手动重启（详见[第八章](/hello-claw/cn/adopt/chapter8/#配置热更新)）。
...
> **按需扩展**：以上只是常见场景的示例。你可以根据自己的使用习惯，添加更多场景处理规则，比如"翻译类需求""购物比价类需求""旅行规划类需求"等。场景越具体，龙虾的执行越精准。进阶用户还可以添加"代码与 PR 类需求"等开发场景（详见[Vibe Coding 实战](/hello-claw/cn/university/vibe-coding/)）。
```
...
| 路径 | 内容 |
|------|------|
| `~/.openclaw/` 配置文件 | 全局配置（详见[附录 G](/hello-claw/cn/appendix/appendix-g)） |
