...
next:
  text: '第3章 初始配置向导'
  link: '/hello-claw/cn/adopt/chapter3'
---
...
> **想跳过这些？** [第一章](/hello-claw/cn/adopt/chapter1/)介绍了 **AutoClaw**——下载 → 双击 → 注册即用，内置模型和免费额度，无需终端操作。
...
更多云部署方案见[附录 C 类 Claw 方案对比与选型](/hello-claw/cn/appendix/appendix-c)。
...
> **更多提供商**：OpenClaw 支持 15+ 模型提供商（DeepSeek、Qwen、Kimi、GLM、OpenAI、Claude 等），完整列表及获取地址见[附录 E 模型提供商选型指南](/hello-claw/cn/appendix/appendix-e)。

> **想了解配置文件结构？** 向导会自动生成配置文件，通常无需手动修改。如需了解每个字段的含义，详见[附录 G 配置文件详解](/hello-claw/cn/appendix/appendix-g)。
...
> 更多命令见[附录 F 命令速查表](/hello-claw/cn/appendix/appendix-f)。
