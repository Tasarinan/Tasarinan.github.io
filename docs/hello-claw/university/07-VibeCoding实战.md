...
![Vibe Coding 工作流架构](/hello-claw/university/images/vibe-coding/lark-vibe-coding-workflow.png)
...
> **重要前提**：Vibe Coding 需要 OpenClaw Agent 具备命令执行能力（用于调用 Claude Code），因此必须将工具配置档设为 `coding` 或 `full`（默认的 `messaging` 不支持）。详见[第七章 工具与定时任务](/hello-claw/adopt/07-工具与定时任务)。
...
| 条件 | 说明 | 参考 |
|------|------|------|
| 飞书渠道已配置 | OpenClaw 已接入飞书，能收发消息 | [第四章 聊天平台接入](/hello-claw/adopt/04-聊天平台接入) |
| 工具配置档为 coding/full | OpenClaw Agent 需要命令执行权限 | [第七章 工具与定时任务](/hello-claw/adopt/07-工具与定时任务) |
...
4. **配置 Anthropic API Key**
   ...
   >
   > ![Anthropic API Key 创建页面](/hello-claw/university/images/vibe-coding/anthropic-token.png)
   >
...
6. **配置 GitHub 认证**（PAT 方式，推荐新手）
   ...
   ![GitHub Token 创建页面](/hello-claw/university/images/vibe-coding/github-token.png)
...
![用户在飞书发送代码需求](/hello-claw/university/images/vibe-coding/lark-vibe-coding-requirement-1.png)

**② 收到 PR 摘要**：机器人完成后回传 PR 编号、状态、修改文件和添加详情。飞书里的 PR 链接可以直接点击跳转 GitHub。

![机器人回传 PR 摘要与链接](/hello-claw/university/images/vibe-coding/lark-vibe-coding-requirement-2.png)

**③ 审查 diff**：点击 PR 链接在手机浏览器里查看代码变更，确认修改内容是否符合预期。

![GitHub PR 页面的文件 diff 视图](/hello-claw/university/images/vibe-coding/lark-vibe-coding-requirement-3.png)

**④ 确认合并**：回到飞书说一句"合并并清理分支"，机器人执行 merge + 删除分支，全程完成。

![用户确认合并，机器人执行并汇报结果](/hello-claw/university/images/vibe-coding/lark-vibe-coding-requirement-4.png)
...
> **群聊安全提示**：在群聊中使用 Vibe Coding 前，确保群成员都是可信的。恶意用户可能通过 @龙虾 注入危险操作。详见[第十章 安全防护与威胁模型](/hello-claw/adopt/10-安全防护与威胁模型)。
