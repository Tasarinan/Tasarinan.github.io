...
prev:
  text: '附录 E：模型提供商选型指南'
  link: '/hello-claw/appendix/E-模型提供商选型'
next:
  text: '附录 G：配置文件详解'
  link: '/hello-claw/appendix/G-配置文件详解'
---
...
> 前置条件：Node.js >= 22。详见[第 2 章 手动安装](/hello-claw/adopt/02-OpenClaw快速安装)。
...
> 详见[第 3 章 初始配置向导](/hello-claw/adopt/03-初始配置向导)。
...
> 详见[第 11 章 Web 界面与客户端](/hello-claw/adopt/11-Web界面与客户端)。
...
> 详见[第 8 章 Gateway 运维与管理](/hello-claw/adopt/08-网关运维)。
...
> 详见[第 4 章 Chat Provider 配置](/hello-claw/adopt/04-聊天平台接入)。
...
> 安装新技能用 `clawhub` CLI，不是 `openclaw skills`。详见[附录 D 技能开发与发布指南](/hello-claw/appendix/D-技能开发与发布)。
...
> 模型配置详见[第 5 章 模型管理](/hello-claw/adopt/05-模型管理)，提供商对比见[附录 E 模型提供商选型指南](/hello-claw/appendix/E-模型提供商选型)。
...
OpenClaw 的配置可以通过 CLI 直接读写（配置文件结构详见[附录 G](/hello-claw/appendix/G-配置文件详解)）。
...
> 配置文件完整说明见[附录 G](/hello-claw/appendix/G-配置文件详解)。
...
> 安全相关详见[第 10 章 安全防护与威胁模型](/hello-claw/adopt/10-安全防护与威胁模型)。
...
> 沙箱详见[第 8 章 Gateway 运维](/hello-claw/adopt/08-网关运维)和[第 10 章 安全防护](/hello-claw/adopt/10-安全防护与威胁模型)。
