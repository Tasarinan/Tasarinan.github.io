...
> 渠道配置详见[第 4 章 Chat Provider 配置](/hello-claw/adopt/04-聊天平台接入)。
...
> 网关管理详见[第 8 章](/hello-claw/adopt/08-网关运维)，远程访问详见[第 9 章](/hello-claw/adopt/09-远程访问与网络)。
...
> 技能管理用 `clawhub` CLI，详见[附录 D 技能开发与发布指南](/hello-claw/appendix/D-技能开发与发布)。
...
> 定时任务的增删改查见[附录 F 命令速查表](/hello-claw/appendix/F-命令速查表#定时任务)。
